...
setlocal indentexpr=GetDartIndent()
...
" GetDartIndent does to find the containing [[{(] (side-effects)
function s:SkipFunc()
  if s:top_col == 1
    throw 'out of bounds'
  elseif s:check_in
...
"   let indentTo = indent(previousLine)

"   if previousLine =~ '[[({]'
"     return indentTo + shiftwidth()
"   endif
